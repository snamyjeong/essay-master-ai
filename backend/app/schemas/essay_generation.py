from pydantic import BaseModel, Field # Pydantic BaseModel 및 Field 임포트
from datetime import datetime # 날짜/시간 타입 임포트
from typing import Optional, List, Any, Dict # 타입 힌트 임포트
...
class EssayBase(BaseModel):
    title: str = Field(..., description="에세이 제목") # 에세이 제목 필드
    content: str = Field(..., description="에세이 내용") # 에세이 내용 필드

# 에세이 생성 요청 스키마
class EssayCreate(EssayBase):
    pass # EssayBase의 필드를 그대로 사용

# 에세이 업데이트 요청 스키마
class EssayUpdateRequest(BaseModel):
    title: Optional[str] = Field(None, description="업데이트할 에세이 제목") # 선택적 제목 필드
    content: Optional[str] = Field(None, description="업데이트할 에세이 내용") # 선택적 내용 필드

# 생성 결과 응답 스키마
class GenerationResultResponse(BaseModel):
    id: int # 생성 결과 ID
    essay_id: int # 관련 에세이 ID
    result_type: str # 결과 유형
    feedback: Optional[str] # 피드백 내용
    score: Optional[float] # 점수
    created_at: datetime # 생성 시각

    class Config:
        from_attributes = True # ORM 객체 매핑 설정

# 에세이 응답 스키마
class EssayResponse(EssayBase):
    id: int # 에세이 ID
    created_at: datetime # 생성 시각
    generations: List[GenerationResultResponse] = [] # 생성 결과 리스트 (기본값 빈 리스트)

    class Config:
        from_attributes = True # ORM 객체 매핑 설정

# 심화 논술 평가 응답 스키마 (evaluate_essay_answer_with_rag 결과 파싱용)
class EssayEvaluationResponse(BaseModel):
    overall_feedback: str = Field(..., description="전반적인 평가 요약") # 전반적인 평가 피드백
    factual_accuracy: Dict[str, Any] = Field(..., description="사실 관계 정확성 평가") # 사실 관계 정확성 평가 결과
    logical_coherence: Dict[str, Any] = Field(..., description="논리적 일관성 평가") # 논리적 일관성 평가 결과
    expression_readability: Dict[str, Any] = Field(..., description="표현력 및 가독성 평가") # 표현력 및 가독성 평가 결과
    overall_score: int = Field(..., ge=0, le=100, description="최종 종합 점수 (0-100)") # 최종 종합 점수
...
class QuizGenerationService: # [교정] 라우터(learning.py)의 호출 규격에 맞춰 클래스명을 정의합니다.
    ...
    def __init__(self, db=None):
        ...
        self.keyword_quiz_prompt = ChatPromptTemplate.from_messages([
            ("system", """당신은 30년 경력의 베테랑 일타 강사 정성남입니다.
            학습 내용에서 핵심 키워드를 추출하여 학생들이 암기하기 좋은 **단답형 문제** 3개를 생성하세요.
            형식: - 문제: [내용] / 정답: [키워드]"""),
            ("user", "학습 내용:\n{content}")
        ])

        # 2. 논술형/서술형 퀴즈 생성 프롬프트 템플릿
        self.essay_quiz_prompt = ChatPromptTemplate.from_messages([
            ("system", """당신은 30년 경력의 일타 강사 정성남입니다.
            심층적인 사고력을 요구하는 **논술형 문제** 1개를 생성하고,
        ...
        # 4. 일타 강사의 멘토링 메시지 생성 프롬프트 템플릿
        self.mentoring_prompt = ChatPromptTemplate.from_messages([
            ("system", "학생의 학습 의욕을 고취시키는 30년 경력 강사의 **따뜻하고 날카로운 한마디**를 작성하세요."),
            ("user", "학습 내용 요약:\n{content}")
        ])
        ...
        self.essay_evaluation_prompt = ChatPromptTemplate.from_messages([
            ("system", """당신은 30년 경력의 일타 강사이자, 채점관입니다.
